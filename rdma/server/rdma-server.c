...
const int data_size = 1024*1024*1024;// * 1024 * 1024; /*1GB*/ 256个key value*4=1024MB=1GB
const int block_size = 1024*1024*4;//一个value的大小，picture经过base64的字符1MB
...
void register_memory_server(struct connection_server *conn)
{
  ...
  {
    //int key=1;
    redisReply* reply = redisCommand(redis_conn, "get %d",3);     //printf("%s\n",reply->str);
...
int main(int argc, char **argv)
{
  ...
  addr.sin_port = htons(12345);//绑定了12345port

  TEST_Z(ec = rdma_create_event_channel());
  TEST_NZ(rdma_create_id(ec, &listener, NULL, RDMA_PS_TCP));
  TEST_NZ(rdma_bind_addr(listener, (struct sockaddr *)&addr));
  TEST_NZ(rdma_listen(listener, 10)); /* backlog=10 is arbitrary 传入连接请求的积压*/
  //ntohs 函数将u_short从 TCP/IP 网络字节顺序转换为托管字节顺序 (，这是 Intel 处理器) 上的小端。
